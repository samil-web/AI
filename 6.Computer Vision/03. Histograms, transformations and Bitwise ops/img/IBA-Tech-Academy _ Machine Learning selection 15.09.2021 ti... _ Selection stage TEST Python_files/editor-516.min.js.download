...
dir=diff<0?1:-1}}while(steps>1||((dir<0)!=(diff<0)&&(Math.abs(diff)<=Math.abs(prevDiff))))
...
cm.indentLine(cur,"smart");cm.setSelection(from,cm.getCursor(false));});});})();(function(mod){if(typeof exports=="object"&&typeof module=="object")
mod(require("../../lib/codemirror"));else if(typeof define=="function"&&define.amd)
define(["../../lib/codemirror"],mod);else
...
function adjustPos(orig,folded,pos){if(orig.length==folded.length)return pos
for(var pos1=Math.min(pos,orig.length);;){var len1=orig.slice(0,pos1).toLowerCase().length
if(len1<pos)++pos1
...
return{from:Pos(line,adjustPos(orig,string,found)+ch),to:Pos(line,adjustPos(orig,string,found+lines[0].length)+ch)}}else{var cutFrom=string.length-lines[0].length
if(string.slice(cutFrom)!=lines[0])continue search
for(var i=1;i<lines.length-1;i++)
if(fold(doc.getLine(line+i))!=lines[i])continue search
var end=doc.getLine(line+lines.length-1),endString=fold(end),lastLine=lines[lines.length-1]
if(end.slice(0,lastLine.length)!=lastLine)continue search
return{from:Pos(line,adjustPos(orig,string,cutFrom)+ch),to:Pos(line+lines.length-1,adjustPos(end,endString,lastLine.length))}}}}
...
if(found==-1)continue search
return{from:Pos(line,adjustPos(orig,string,found)),to:Pos(line,adjustPos(orig,string,found+lines[0].length))}}else{var lastLine=lines[lines.length-1]
if(string.slice(0,lastLine.length)!=lastLine)continue search
for(var i=1,start=line-lines.length+1;i<lines.length-1;i++)
if(fold(doc.getLine(start+i))!=lines[i])continue search
var top=doc.getLine(line+1-lines.length),topString=fold(top)
if(topString.slice(topString.length-lines[0].length)!=lines[0])continue search
return{from:Pos(line+1-lines.length,adjustPos(top,topString,top.length-lines[0].length)),to:Pos(line,adjustPos(orig,string,lastLine.length))}}}}
...
CodeMirror.commands.jumpToLine=function(cm){var cur=cm.getCursor();dialog(cm,jumpDialog,"Jump to line:",(cur.line+1)+":"+cur.ch,function(posStr){if(!posStr)return;var match;if(match=/^\s*([\+\-]?\d+)\s*\:\s*(\d+)\s*$/.exec(posStr)){cm.setCursor(interpretLine(cm,match[1]),Number(match[2]))}else if(match=/^\s*([\+\-]?\d+(\.\d+)?)\%\s*/.exec(posStr)){var line=Math.round(cm.lineCount()*Number(match[1])/ 100);if(/^[-+]/.test(match[1]))line=cur.line+line+1;cm.setCursor(line-1,cur.ch);}else if(match=/^\s*\:?\s*([\+\-]?\d+)\s*/.exec(posStr)){cm.setCursor(interpretLine(cm,match[1]),cur.ch);}});};CodeMirror.keyMap["default"]["Alt-G"]="jumpToLine";});(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(window.jQuery);}}(function($){if(!Array.prototype.reduce){Array.prototype.reduce=function(callback){var t=Object(this),len=t.length>>>0,k=0,value;if(arguments.length===2){value=arguments[1];}else{while(k<len&&!(k in t)){k++;}
...
selection.addRange(nativeRng);}else{nativeRng.select();}};this.normalize=function(){var getVisiblePoint=function(point,isLeftToRight){if((dom.isVisiblePoint(point)&&!dom.isEdgePoint(point))||(dom.isVisiblePoint(point)&&dom.isRightEdgePoint(point)&&!isLeftToRight)||(dom.isVisiblePoint(point)&&dom.isLeftEdgePoint(point)&&isLeftToRight)||(dom.isVisiblePoint(point)&&dom.isBlock(point.node)&&dom.isEmpty(point.node))){return point;}
var block=dom.ancestor(point.node,dom.isBlock);if(((dom.isLeftEdgePointOf(point,block)||dom.isVoid(dom.prevPoint(point).node))&&!isLeftToRight)||((dom.isRightEdgePointOf(point,block)||dom.isVoid(dom.nextPoint(point).node))&&isLeftToRight)){if(dom.isVisiblePoint(point)){return point;}
isLeftToRight=!isLeftToRight;}
var nextPoint=isLeftToRight?dom.nextPointUntil(dom.nextPoint(point),dom.isVisiblePoint):dom.prevPointUntil(dom.prevPoint(point),dom.isVisiblePoint);return nextPoint||point;};var startPoint=getVisiblePoint(this.getStartPoint());var endPoint=getVisiblePoint(this.getEndPoint());return new WrappedRange(startPoint.node,startPoint.offset,endPoint.node,endPoint.offset);};this.nodes=function(pred,options){pred=pred||func.ok;var includeAncestor=options&&options.includeAncestor;var fullyContains=options&&options.fullyContains;var startPoint=this.getStartPoint();var endPoint=this.getEndPoint();var nodes=[];var leftEdgeNodes=[];dom.walkPoint(startPoint,endPoint,function(point){if(dom.isEditable(point.node)){return;}
...
this.each(function(i,holder){var info=renderer.layoutInfoFromHolder($(holder));var isCodeview=info.editor.hasClass('codeview');if(info&&info.editable){if(isCodeview){info.codable.val(sHTML);if(agent.hasCodeMirror){info.codable.data('cmEditor').setValue(sHTML);}}
else{info.editable.html(sHTML);}}});return this;},destroy:function(){this.each(function(idx,holder){var $holder=$(holder);var info=renderer.layoutInfoFromHolder($holder);if(!info||!info.editable){return;}
var options=info.editor.data('options');eventHandler.detach(info,options);renderer.removeLayout($holder,info,options);});return this;}});}));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(window.jQuery);}}(function($){var tmpl=$.summernote.renderer.getTemplate();var range=$.summernote.core.range;var dom=$.summernote.core.dom;var getTextOnRange=function($editable){$editable.focus();var rng=range.create();if(rng.isOnAnchor()){var anchor=dom.ancestor(rng.sc,dom.isAnchor);rng=range.createFromNode(anchor);}
return rng.toString();};var toggleBtn=function($btn,isEnable){$btn.toggleClass('disabled',!isEnable);$btn.attr('disabled',!isEnable);};var showTalentImageDialog=function($editable,$dialog){return $.Deferred(function(deferred){var $imageDialog=$dialog.find('.note-talent-image-dialog');var $imageUrl=$imageDialog.find('.note-image-url'),$imageSelect=$imageDialog.find('.imagesselect'),$okBtn=$imageDialog.find('.note-ok-btn');$imageDialog.one('shown.bs.modal',function(){$imageUrl.on('input',function(){toggleBtn($okBtn,$imageUrl.val());}).trigger('focus');$okBtn.click(function(event){event.preventDefault();$editable.focus();deferred.resolve($imageUrl.val());$imageDialog.modal('hide');});$('.imagesselect').change(function(){fid=(showSelect2)?$imageSelect.select2('val'):$imageSelect.val();if(fid==0){return;}
fid=(showSelect2)?$imageSelect.select2('val'):$imageSelect.val();url=$('textarea.editor').attr('data-selectedurl').replace("##fileid##",fid);myportal.app.ajax(url,{type:"post",data:{},success:function(response){var data=$.parseJSON(response).data;myportal.app.filescache.fileurls[data.id]=data.url;myportal.app.filescache.filetypes[data.id]=data.content_type;myportal.app.filescache.fileids[decodeURIComponent(data.url)]=data.id;$editable.focus();deferred.resolve(decodeURIComponent(data.url));$imageDialog.modal('hide');}});}).trigger('focus');}).one('hidden.bs.modal',function(){(showSelect2)?$imageSelect.select2('val',0):$imageSelect.val(0);$('.note-image-url').val('');$('.imagesselect').off('change');$imageUrl.off('input');$okBtn.off('click');if(deferred.state()==='pending'){deferred.reject();}}).modal('show');});};$.summernote.addPlugin({name:'talentImage',buttons:{talentImage:function(lang){return tmpl.iconButton('fa fa-file-picture-o',{event:'showTalentImageDialog',title:'Image',hide:true});}},dialogs:{talentImage:function(lang){var imageSelectId='imagesselect_'+Math.round(new Date().getTime()/ 1000);var body='<div class="pull-left" style="margin-right: 5px;">'+'<select class="imagesselect" id="'+imageSelectId+'"><option value="0">'+myportal.app.translate('image-course-files')+'</option></select>'+'</div>';var script='';if(!$("#template-upload").length){script="<script>"+"var fileUploadErrors = {"+"maxFileSize: 'File is too big',"+"minFileSize: 'File is too small',"+"acceptFileTypes: 'File type not accepted',"+"maxNumberOfFiles: 'Max number of files exceeded',"+"uploadedBytes: 'Uploaded bytes exceed file size',"+"emptyResult: 'Empty file upload result'"+"};"+"</script>"+"<script id='template-upload' type='text/html'>"+"{% for (var i=0, file; file=o.files[i]; i++) { %}"+"<tr class='template-upload fade'>"+"<td>"+"<strong class='error text-danger'></strong>"+"<div class='progress progress-striped active' role='progressbar' aria-valuemin='0' aria-valuemax='100' aria-valuenow='0' style='margin-left:5px;max-width:200px'>"+"<div class='bar bar-success progress-bar progress-bar-success' style='width:0%;'></div>"+"</div>"+"</td>"+"</tr>"+"{% } %}"+"</script>"+"<script id='template-download' type='text/html'>"+"{% for (var i=0, files=o.files, l=files.length, file=files[0]; i<l; file=files[++i]) { %}"+"<tr class='template-download fade'>"+"{% if (file.error) { %}"+"<td></td>"+"<td class='error' colspan='2'><span class='label important'>Error</span> {%=fileUploadErrors[file.error] || file.error%}</td>"+"{% } else { %}"+"<td class='preview' style='text-align: center;vertical-align:middle'>{% if (file.thumbnail_url) { %}"+"<img src='{%=file.thumbnail_url%}'>"+"{% } %}</td>"+"{% } %}"+"</tr>"+"{% } %}"+"</script>";}
var imageUploadNamespace='images_namespace_'+Math.round(new Date().getTime()/ 1000);var uploader="<div style='max-height: 20px;margin-top: 2px;' id='"+imageUploadNamespace+"' class='tl-editor-uploader' data-selectbox='"+imageSelectId+"' data-url='"+$('textarea.editor').data('uploadimagesurl')+"'>"+"<span class='file-wrapper fileinput-button'>"+"<input type='file' multiple='' name='files[]'>"+"<a class='tl-editor-uploader-tool-tip none-decoration' tabindex='-1' rel='tooltip' data-original-title='<u>"+myportal.app.translate('accepted-files')+"</u>: "+editorFormattedAcceptedFileTypes+"<br><u>"+myportal.app.translate('max-file-size')+"</u>: "+editorFormattedMaxUploadSize+"' data-html='true' id='fileupload_input' style='float:left; line-height: 18px; font-size: 13px;' href='javascript:void(0);'>"+"<i class='icon-upload icon-grid'></i>"+"</a><div class='note error text-error pull-left' style='margin-left:3px'> </div>"+"</span>"+"<table class='zebra-striped' style='width: 40%;'>"+"<tbody class='files'></tbody>"+"</table>"+
script+"</span>"+"</div>";var url_input='<div class="clear"></div><div class="form-group row-fluid"><input class="note-image-url form-control span8" type="text" placeholder="'+myportal.app.translate('use-image-url')+'">'+'<button href="#" class="btn btn-primary note-ok-btn disabled" style="margin-left:5px" disabled>OK</button></div>';return tmpl.dialog('note-talent-image-dialog',"Insert Image",body+uploader+url_input,' ');}},events:{showTalentImageDialog:function(event,editor,layoutInfo){var $dialog=layoutInfo.dialog(),$editable=layoutInfo.editable();editor.saveNode($editable);editor.saveRange($editable);showTalentImageDialog($editable,$dialog).then(function(url){editor.restoreRange($editable);editor.insertImage($editable,url);editor.afterCommand($editable);}).fail(function(){});}}});}));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(window.jQuery);}}(function($){var tmpl=$.summernote.renderer.getTemplate();$.summernote.addPlugin({name:'talentSmartTags',buttons:{talentSmartTagsDropdown:function(){var list='';if(typeof smartTags!=='undefined'){for(var key in smartTags){list+='<li><a data-event="talentSmartTagsDropdown" href="javascript:void(0);" data-value="'+key+'">'+smartTags[key]+'</a></li>';}}
var dropdown='<ul class="dropdown-menu">'+list+'</ul>';return tmpl.iconButton('fa fa-cogs',{title:'Smart Tags',hide:true,dropdown:dropdown});}},events:{talentSmartTagsDropdown:function(event,editor,layoutInfo,value){var $editable=layoutInfo.editable();editor.insertText($editable,value);}}});}));